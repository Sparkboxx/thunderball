...
GyroDataMapper::GyroDataMapper(Gyro* g){
  gyro = g;
};
...
void GyroDataMapper::parse_yaw(){
  int result = ((data[3]>>2)<<8)+data[0];
  gyro->set_yaw(result);
};

void GyroDataMapper::parse_pitch(){
  int result = ((data[5]>>2)<<8)+data[2];
  gyro->set_pitch(result);
};

void GyroDataMapper::parse_roll(){
  int result = ((data[4]>>2)<<8)+data[1];
  gyro->set_roll(result);
};

void GyroDataMapper::parse_yaw_slow(){
  int result = (data[3] >> 1) & 0x0001;
  gyro->set_yaw_slow(result);
};

void GyroDataMapper::parse_pitch_slow(){
  gyro->set_pitch_slow(data[3] & 0x0001);
};

void GyroDataMapper::parse_roll_slow(){
  gyro->set_roll_slow((data[4] >> 1) & 0x0001);
};

bool GyroDataMapper::set_extension_connected(){
  return extension_connected = data[4] & 0x0001;
};
